...
const rgbMean = (a, b) => {
    const cols = ["r", "g", "b"];
    const meanCol = cols.map((col) => Math.sqrt((Math.pow(a[col], 2) + Math.pow(b[col], 2)) / 2));
    let opacity;
...
const Mark = ({ color, className, end, start, onClick, content, tag, }) => {
    ...
    return (React.createElement("mark", { className: className, style: Object.assign({ backgroundColor: color || "#84d2ff", padding: "0 4px" }, (lumin && { color: "white" })), "data-start": start, "data-end": end, onMouseUp: () => onClick({ start: start, end: end }) },
        content,
        tag && (React.createElement("span", { style: { fontSize: "0.7em", fontWeight: 500, marginLeft: 6 } }, tag))));
};
...
const focusOverlap = (baseTag, splits) => {
    const valA = baseTag;
    const overlap = splits
        .map((valB) => {
        if (valA.color) {
            if (valA.end >= valB.start &&
                valA.end <= valB.end &&
...
const splitOneGap = (tagRange) => {
    const breaks = tagRange
        .map((i, j) => {
        if (i === -1) {
            return j;
        }
...
const updateIndices = (splits, blend) => {
    const metaData = [];
    const metaIndex = [];
    ...
    const tagRanges = splits
        ...
        const semiInclusive = blendRange.includes(i.start) || blendRange.includes(i.end);
        const totalInclusive = tagRange.every((val) => blendRange.includes(val));
        const blendInclusive = blendRange.every((val) => tagRange.includes(val));
        ...
        if ((semiInclusive && !totalInclusive) ||
            ...
            const inclusive = i;
            totalIncInds.push(splits.findIndex((tag, index) => tag.start <= inclusive.start &&
    ...
    for (let val of totalIncInds) {
        if (metaIndex.every((meta) => meta !== val) && val >= 0) {
            ...
            blend.forEach((overlap) => {
                let start = 0;
                let end = 0;
                if (overlap.end - overlap.start === 1 &&
                ...
                [...parentTag] = parentTag.filter((i) => !range(start, end).includes(i));
            });
...
const tagFilter = (tags) => {
    const filterSet = tags.outRanges.flatMap((val, j) => {
        ...
        return tagVal.map((tag) => (Object.assign(Object.assign({}, tagMeta), { start: Math.min(...tag), end: Math.max(...tag) })));
    });
    return filterSet.filter((tag) => isFinite(tag.start) || isFinite(tag.end));
};
const blender = (tags) => {
    const currentTags = lodash_sortby(tags, ["start"]);
